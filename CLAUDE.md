...
- Vindlinjer (statiska, ingen animation, inspirerat av Apple Väder): loadWindField() körs en gång efter väderhämtningen i selectLocation() — ett Open-Meteo-anrop med 7×7 = 49 koordinater (kommaseparerade) över kartvyn pad(0.5), hourly wind_speed/direction, samma timezone/forecast_days som huvudanropet (S.hourly.tz/days) så att timindex matchar
- renderWindLines(): strömlinjer genom bilinjärt interpolerat fält, frön var 78:e px (fast PRNG → samma mönster vid omritning), längd 30–100 px efter vindstyrka, 5 segment med ökande opacitet/tjocklek (svans → huvud visar riktningen). Ritas på en canvas (L.canvas) i egen pane 'windPane' (z 350, mellan tiles och markörer). Ritas om utan nytt anrop vid prognosslider (renderAll) och zoomend
- windFieldReq-räknare förhindrar att ett sent svar ritar linjer för en gammal plats. Misslyckat anrop ignoreras tyst (linjerna är dekor)
